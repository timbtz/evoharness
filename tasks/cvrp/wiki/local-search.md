...
The seed's kernel applies this restricted to K-nearest candidate pairs.
...
Apply if insert_cost < removal_gain. The seed's kernel searches L<=3, both
orientations, near K-nearest neighbors. Capacity precheck BEFORE
...
## 2-opt* (tail exchange — strongest inter-route move)
...
This move reshapes route boundaries that relocate/swap/2-opt cannot reach; the
seed's kernel searches both tail pairings of each candidate arc.
