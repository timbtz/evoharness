...
class Config:
    ...
    refiner: str | None = None  # z.ai model id: every refiner_every writer candidates
    # one session sees the whole window (ideas, feedback, code) + the wiki and
    # synthesizes ONE v2 (evaluated + gated like any candidate; id suffix "f").
    # 2026-07-27: this used to be a headless Claude session on a separate bill;
    # all roles now run on z.ai and are charged to the run's budget guard.
    ...
    analyst: str | None = None  # z.ai model id: independent in-run analysis session
    # every analyst_every candidates; writes new-ideas/analyst-*.md to the wiki
    analyst_every: int = 10
    analyst_web: bool = False  # give the analyst web_search/fetch_url: it can pull
    # approaches beyond the wiki (papers, write-ups) into its proposals
    analyst_inject: int = 0  # >0: up to N candidate modules per analyst session are
    # parsed out of its reply and written to <run_dir>/INJECT/; the loop evaluates +
    # gates them like writer candidates at the next generation (ids c<gen>i<k>)
